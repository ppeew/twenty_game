...
func (d TradeDao) DeleteByID(id int, userID uint32) error {
	...
	// 3.解锁物品
	exec := begin.Exec("update user_and_item set lock_nums = ? where user_id = ? and item_id = ?", gorm.Expr("lock_nums - ?", tradeItem.Count), userID, id)
	if exec.RowsAffected == 0 {
		//找不到记录,也即是用户没有该物品
		begin.Rollback()
		return errors.New("商品数量不足上架")
	}
	begin.Commit()
	return nil
}
...
func (d TradeDao) BuyTradeItem(TradeID int, userID int) error {
	...
	tx := begin.Raw("select a.item_nums from user_and_item a inner join users b on a.user_id = b.id where a.item_id = ? AND b.id=?", TradeID, userID).Scan(&m)
	if tx.RowsAffected == 0 {
		//如果找不到该记录，则添加
		exec := begin.Exec("insert into user_and_item (`user_id`,`item_id`,`item_nums`,`lock_nums`) values (?,?,?,0)", userID, TradeID, tradeItem.Count)
		if exec.RowsAffected == 0 {
			//出现问题回滚一切操作
			begin.Rollback()
			return errors.New("服务器错误")
		...
		//否则就是找到了记录，在原来数量添加
		exec := begin.Exec("update user_and_item set item_nums = ? where user_id=? and item_id=?", gorm.Expr("item_nums+?", tradeItem.Count), userID, TradeID)
		if exec.RowsAffected == 0 {
			//出现问题回滚一切操作
			begin.Rollback()
			return errors.New("服务器错误")
